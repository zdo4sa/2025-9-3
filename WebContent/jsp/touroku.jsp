...
<div class="container">
	<h1>お問い合わせフォーム</h1>
	<form action="${inquiry1Url}" method="post" onsubmit="return validateForm()" enctype="multipart/form-data">
		<p>
			<label for="name">名前:</label>
			<input type="text" id="name" name="name" value="<c:out value='${inquiry1.name}'/>">
			<span class="error-message"><c:out value='${errors.name}' /></span>
		</p>
		<p>
			<label for="email">メールアドレス:</label>
			<input type="email" id="email" name="email" value="<c:out value='${inquiry1.email}'/>">
			<span class="error-message"><c:out value='${errors.email}' /></span>
		</p>
		<p>
			<label for="content">内容:</label>
			<textarea id="content" name="content" rows="5" cols="40"><c:out value='${inquiry1.content}' /></textarea>
